...
// Rotas Alunos (mantidas como você enviou)
...
// Rota para listar disciplinas (corrigida para usar db.execute e retornar id e nome)
app.get('/disciplinas', async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT id, nome FROM disciplinas ORDER BY nome');
    res.json(rows);
  } catch (error) {
...
// Rota para buscar notas do aluno incluindo nome da disciplina
app.get('/notas/aluno/:aluno_id', async (req, res) => {
  try {
    // Faz join para pegar o nome da disciplina junto da nota
...
// Editar nota (corrigida para atualizar todos os campos necessários)
